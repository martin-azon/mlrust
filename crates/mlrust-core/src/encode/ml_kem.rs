...
use crate::params::{RingParams, Q3329, N};
...
pub fn byte_encode_poly_q3329<const D: usize>(
    poly: &Poly<Q3329>,
    out: &mut [u8],
) {
    assert!(D > 0);
...
#[must_use]
pub fn byte_decode_poly_q3329<const D: usize>(
    input: &[u8],
) -> Poly<Q3329> {
    assert!(D > 0);
    assert!(D <= 12);
