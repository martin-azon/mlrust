...
>(
    ...
    let kpke_keypair = kpke_keygen::<K, EK_PKE_BYTES, DK_PKE_BYTES, ETA1>(randomness_d);
    ...
    dk_bytes[(DK_PKE_BYTES + EK_BYTES)..(DK_PKE_BYTES + EK_BYTES + 32)].copy_from_slice(&hash);
...
>(
    ...
    let encr_key = KpkeEncryptionKey::<EK_BYTES>::from_bytes(*ek.as_bytes());

    let ciphertext = kpke_encrypt::<K, EK_BYTES, CT_BYTES, ETA1, ETA2, DU, DV>(
        &encr_key,
        randomness_m,
        &randomness_r,
    );
...
>(
    ...
    ek_pke_bytes.copy_from_slice(&dk_bytes[DK_PKE_BYTES..(DK_PKE_BYTES + EK_PKE_BYTES)]);

    hash.copy_from_slice(
        &dk_bytes[(DK_PKE_BYTES + EK_PKE_BYTES)..(DK_PKE_BYTES + EK_PKE_BYTES + 32)],
    );

    randomness_z.copy_from_slice(&dk_bytes[(DK_PKE_BYTES + EK_PKE_BYTES + 32)..]);
    ...
    let encr_key = KpkeEncryptionKey::<EK_PKE_BYTES>::from_bytes(ek_pke_bytes);

    let alternative_ciphertext = kpke_encrypt::<K, EK_PKE_BYTES, CT_BYTES, ETA1, ETA2, DU, DV>(
        &encr_key,
        &randomness_m,
        &randomness_r,
    );
