//! NTT constants for the ML-DSA modulus q = 8380417.
//!
//! The tables in this module follow the FIPS 204 Appendix B exponent order,
//! but their entries are stored in Montgomery representation.

/// FIPS 204 Appendix B zeta table converted to Montgomery representation.
/// Each entry is:
///
/// ```text
/// zeta^BitRev_8(i) * R mod q,
/// ```
///
/// where:
/// q = 8380417
...
/// zeta = 1753
///
/// The entry at index `0` is the dummy value `0`, matching the FIPS table.
/// The NTT algorithms use `zetas[1..256]`.
pub const FIPS_ZETAS_MONT: [i32; 256] = [
    0, 25847, 5771523, 7861508, 237124, 7602457, 7504169, 466468, 1826347, 2353451, 8021166,
...
/// This is 256^(-1) * 4193792 mod 8380417 = 16382
pub const INV_NTT_SCALE_MONT: i32 = 16382;
...
mod tests {
    ...
    const ZETA: i32 = 1_753;
    const R_MOD_Q: i32 = 4_193_792;
    ...
    fn inv_ntt_scale_mont_is_correct() {
        let inv_256 = 8347681;
        let expected = mont_encode(inv_256);

        assert_eq!(INV_NTT_SCALE_MONT, expected);
        assert_eq!(INV_NTT_SCALE_MONT, 16382);
    }

    #[test]
    fn fips_zetas_mont_match_formula() {
        for i in 1..256 {
            let exponent = bitrev8(i);
